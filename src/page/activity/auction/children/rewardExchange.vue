<template>
    <div class="reward_exchange">
        <div class="header">
            <img :src="goodsPic.homeAndtakePrize" alt="" width="100%">
        </div>
        <div class="content">
            <div class="form_box">
                <form>
...
<script>
    import { exchange, goodsPic } from '../../../../service/getData'
    export default {
        data () {
            return {
                showShare: true,
            ...
            submitForm (user) {
                ...
                exchange(that.bidId, user.wxNum, user.phoneNum).then(function (respones) {
                    if (respones && respones.errno === 0) {
                        that.$parent.showErrMsg('兑换成功',true)
                        that.user = {
                            phoneNum: '',
                            wxNum: ''
                        }
                    } else {
                        that.$parent.showErrMsg(respones.errmsg)
                    }
                })
            },
